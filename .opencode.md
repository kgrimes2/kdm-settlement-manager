...
**CRITICAL:** Under NO circumstances should OpenCode:
- ❌ **Merge pull requests** - PRs must be manually reviewed and merged by humans
- ❌ **Deploy to production** - Production deployments require human approval
- ❌ **Run `terraform apply` on production** - NEVER, EVER run terraform apply for prod, even with `-target` or `-auto-approve`
- ❌ **Run `terraform apply` in prod workspace** - Even partial/targeted applies are forbidden
- ❌ **Approve production deployments** - All prod approvals must be manual

**Allowed production operations:**
- ✅ **`terraform plan` for production** - Planning/previewing prod changes is safe
- ✅ **`terraform apply` for non-prod environments (dev/staging)** - Non-prod changes allowed with explicit approval
- ✅ **Show user the exact apply command for prod** - User must run it themselves
...
```
📊 Session: XXX,XXX tokens used ($X.XX) | Total: $X.XX
```

**Example:**
```
📊 Session: 120,500 tokens used ($0.36) | Total: $1.24
```

### Detailed Report Format (when milestones are hit)
When reaching usage milestones (50%, 75%, 90%), provide detailed breakdown:
```
📊 Token Usage: XX,XXX / 200,000 (XX.X%)
...
**Calculation method:**
- Track tokens from system warnings
- Assume ~95% input / ~5% output ratio (tool-heavy usage)
- Calculate: (input × $3 + output × $15) / 1,000,000

**Note:** Actual costs may vary by region and usage patterns. These are estimates based on standard Bedrock pricing.
...
**Changes:**
- **1.2.0:** Added safety constraints for PR merging and production deployments
- **1.1.0:** Added token usage tracking and reporting guidelines
